...
for i in ${testnet[@]}; do
  ssh -p 22008 java-tron@$i 'rm -rf /data/databackup/java-tron/output-directory/ && rm -rf /data/databackup/java-tron/liteDatabase/output-directory/'
  echo "Delete database file of ${i} completed"
 # ssh -p 22008 java-tron@$i 'rm -rf /data/databackup/java-tron/liteDatabase/output-directory/'
done

cd /data/FullNode/liteDatabase
for node in ${testnet[@]}; do {
tar -c output-directory/ |pigz |ssh -p 22008 java-tron@$node "gzip -d|tar -xC /data/databackup/java-tron/liteDatabase/" > /data/workspace/replay_workspace/server_workspace/${node}DBsend.log
} &
