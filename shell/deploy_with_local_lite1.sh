...
#FullNode 的不同端口节点，用于部署一个机器上部署多个FullNode
duplicateNode=true
...
sendDuplicateNode() {
  for i in ${duplicateNodeIp[@]}; do
    ...
    tar -c java-tron-1.0.0/ |pigz |ssh -p 22008 java-tron@$i "gzip -d|tar -xC /data/databackup/java-tron-copy/"
    ...
    scp -P 22008 /data/workspace/replay_workspace/server_workspace/conf/duplicate/start_new_fullnode.sh java-tron@$i:/data/databackup/java-tron-copy/start.sh
    scp -P 22008 /data/workspace/replay_workspace/server_workspace/conf/duplicate/stop_new.sh java-tron@$i:/data/databackup/java-tron-copy/stop.sh
